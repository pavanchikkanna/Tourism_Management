...
app.controller("descriptionController",function($scope,$http,$routeParams){
...
{//validate and store the email in sessionStorage
    ...
    else if(response.data.message=="invalidPassword")
    {
      ...
      $rootScope.sssiginmessage=true;
    ...
    else
        {
          ...
          $window.sessionStorage.setItem("userEmail",response.data.message);
          $window.location.href="index.html";
...
{ //retrieve the basic details of the user and keep track any changes to the input fields
  let data={"email":$window.sessionStorage.getItem("userEmail")}
